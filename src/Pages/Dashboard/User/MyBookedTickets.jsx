...
const MyBookedTickets = () => {
    ...
    const paidCount = userBookings.filter(t => t.paymentStatus === 'paid').length;
    ...
    return (
        <section className="space-y-8 max-w-7xl mx-auto">
            ...
            {/* 1. Page Header & Stats */}
            <div className="flex flex-col md:flex-row items-end justify-between gap-6 pb-6 border-b border-(--border-card)">
                <div>
                ...
                {userBookings.length > 0 && (
                    <div className="flex gap-3">
                        <div className="px-4 py-2 rounded-xl bg-(--bg-card) border border-(--border-card) flex items-center gap-2 shadow-sm">
                            ...
                            <div>
                                <p className="text-[10px] font-bold uppercase text-(--text-muted)">Confirmed</p>
                                <p className="text-lg font-black text-(--text-main) leading-none">{paidCount}</p>
                            </div>
                        </div>
                        <div className="px-4 py-2 rounded-xl bg-(--bg-card) border border-(--border-card) flex items-center gap-2 shadow-sm">
                            ...
                            <div>
                                <p className="text-[10px] font-bold uppercase text-(--text-muted)">Pending</p>
                                <p className="text-lg font-black text-(--text-main) leading-none">{pendingCount}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {userBookings?.length > 0 ? (
                    ...
                    <div className="col-span-full py-24 flex flex-col items-center justify-center text-center bg-(--bg-card) border border-dashed border-(--border-card) rounded-4xl">
                        ...
                        <p className="text-(--text-muted) max-w-sm mx-auto mb-6">
                            You haven't booked any trips yet. Explore our routes to find your next destination.
                        </p>
